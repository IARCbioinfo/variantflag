...
names( snvs )[1] <- "X.CHROM"

#filter PASS
snvs <- snvs[ FILTER=="PASS", ]

## filter non classical chromosomes
snvs <- snvs[ grep( "chrUn|alt|random|M", X.CHROM, invert = T ) ]
...
foo <- cbind( snvs[,1:5], Cov_N, Cov_T, AF_N, AF_T )
#write.table( foo, file=paste0( substr( args[1], 1, nchar(args[1])-6), "vcf" ), sep="\t", row.names=F, quote=F )
write.table( foo, file=paste0( gsub( "vcf.gz", "vcf", args[1] ) ), sep="\t", row.names=F, quote=F )
